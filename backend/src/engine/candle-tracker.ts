  ...
  processTick(tick: TickData) {
    for (const { key, timeField } of TIMEFRAMES) {
      ...
      if (last !== undefined && current !== last) {
        ...
        const candle = new Date(last).toISOString();
        const detected = new Date().toISOString();
        console.log(`[ENGINE-candle-tracker:${this.brokerName}] timeframe=${key} | symbol=${tick.symbol} | candle=${candle} detected=${detected}`);
