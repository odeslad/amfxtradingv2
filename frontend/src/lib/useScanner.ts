...
export function useScanner() {
  ...
  const run = useCallback(async ({ broker, timeframe, emaFast, emaSlow, recentWithin }: ScannerParams) => {
    if (!broker) return;
    setLoading(true);
    try {
      const url = apiUrl(
        `/scanner?broker=${encodeURIComponent(broker)}&tf=${timeframe}&emaFast=${emaFast}&emaSlow=${emaSlow}&recentWithin=${recentWithin}`,
      );
